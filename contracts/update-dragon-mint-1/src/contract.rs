...
use crate::msg::{
    Claim, ClaimMessage, CollectionInfoResponse, CustomMintMsg, ExecuteMsg, Extension,
    InstantiateMsg, QueryMsg, StateResponse,
};
use crate::state::{
    CollectionInfo, Dragon, DragonListResponse, DragonResponse, State, COLLECTION_INFO,
    DRAGON_INFO, DRAGON_INFO_SEQ, MIN_STAKE_TIME, STATE,
};
...
pub fn instantiate(
    ...
    MIN_STAKE_TIME.save(deps.storage, &Uint64::new(0))?;
...
pub fn execute(
    ...
    match msg {
        ...
        ExecuteMsg::UpdateRewardContractAddress { new_address } => {
            execute_update_reward_contract_address(deps, info, new_address)
        }
...
fn execute_update_reward_contract_address(
    ...
    STATE.save(deps.storage, &state)?;
    Ok(Response::default()
        .add_attribute("new_reward_contract_address", state.reward_contract_address))
}
...
fn execute_mint(
    ...
    Ok(Response::default()
        ...
        .add_attribute(
            ...
            dragon.unstaking_start_time.to_string(),
        )
        .add_attribute(
            "unstaking_process, {}",
            dragon.unstaking_start_time.to_string(),
...
fn execute_transfer_nft(
    ...
    Cw721Contract::default()
        .execute(deps.branch(), env, info.clone(), msg)
        .map_err(|e| ContractError::CW721TransferError {})?;
    let mut dragon = DRAGON_INFO.load(deps.storage, token_id.u64())?;
...
fn execute_claim_reward(
    ...
    if dragon.unstaking_process {
        if dragon.kind == "common".to_string() {
            let second_difference = Uint128::new(dragon.reward_end_time.u64() as u128)
                .checked_sub(Uint128::new(dragon.reward_start_time.u64() as u128))?;
            let second_difference_multiplied =
                second_difference.checked_mul(Uint128::new(500000))?;
            reward = second_difference_multiplied
                .checked_div(Uint128::new(86400))
                .unwrap_or(Uint128::zero());
        } else {
            let second_difference = Uint128::new(dragon.reward_end_time.u64() as u128)
                .checked_sub(Uint128::new(dragon.reward_start_time.u64() as u128))?;
            let second_difference_multiplied =
                second_difference.checked_mul(Uint128::new(1000000))?;
            let second_difference_multiplied_daily_income = second_difference_multiplied
                .checked_mul(Uint128::new(dragon.daily_income.parse::<u128>().unwrap()))?;
            reward = second_difference_multiplied_daily_income
                .checked_div(Uint128::new(86400))
                .unwrap_or(Uint128::zero());
        }
    } else {
        if dragon.kind == "common".to_string() {
            let second_difference = Uint128::new(now as u128)
                .checked_sub(Uint128::new(dragon.reward_start_time.u64() as u128))?;
            let second_difference_multiplied =
                second_difference.checked_mul(Uint128::new(500000))?;
            reward = second_difference_multiplied
    ...
    DRAGON_INFO.save(deps.storage, token_id.u64(), &dragon)?;
    Ok(Response::new().add_submessages(vec![SubMsg::new(claim_reward_msg)]))
}
...
fn query_calculate_reward(deps: Deps, env: Env, token_id: Uint64) -> StdResult<Uint64> {
    ...
    if dragon.unstaking_process {
        if dragon.kind == "common".to_string() {
            let second_difference = dragon
                .reward_end_time
                .checked_sub(dragon.reward_start_time)?;
            let second_difference_multiplied =
                second_difference.checked_mul(Uint64::new(500000))?;
            ...
            let second_difference = dragon
                ...
                .checked_sub(dragon.reward_start_time)?;
            let second_difference_multiplied =
                second_difference.checked_mul(Uint64::new(1000000))?;
            let second_difference_multiplied_daily_income = second_difference_multiplied
                .checked_mul(Uint64::new(dragon.daily_income.parse::<u64>().unwrap()))?;
